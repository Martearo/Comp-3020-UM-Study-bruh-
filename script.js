...
// ----------------------------------------------------
// --- My Reviews Page: Show only user's reviews ---
// ----------------------------------------------------
(function initMyReviewsModule() {
    ...
    function createCardElement(review) {
        ...
        article.innerHTML = `
            ...
            <div class="review-footer">
                ...
                <div class="thumbs">
                    <div class="thumb-display up" aria-hidden="true">👍 <span class="thumb-count">${review.up || 0}</span></div>
                    <div class="thumb-display down" aria-hidden="true">👎 <span class="thumb-count">${review.down || 0}</span></div>
                </div>
            </div>
        `;
    ...
    function applyFiltersAndSort() {
        ...
        if (userReviews.length === 0) {
            reviewsGrid.innerHTML = '<p class="no-reviews">You haven\'t written any reviews yet.</p>';
        ...
        if (searchTerm) {
            filteredReviews = userReviews.filter(review => 
                ...
                (review.roomId && review.roomId.toLowerCase().includes(searchTerm))
            );
        }
        ...
        if (minUp > 0) {
            filteredReviews = filteredReviews.filter(review => (review.up || 0) >= minUp);
        }
        ...
        filteredReviews.sort((a, b) => {
            ...
            if (key === 'up') {
                return dir === 'asc' ? (a.up || 0) - (b.up || 0) : (b.up || 0) - (a.up || 0);
            }
            if (key === 'location') {
